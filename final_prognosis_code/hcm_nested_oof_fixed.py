...
Notes:
    - No ROI segmentation is required.
...
RUN_CONFIG = {
    ...
    "visible_gpus": "0,1,2,3",      # HCM-only run uses all four physical GPUs
    ...
    # "split_source_dir": "/data/projects/MRI/20260705_04_main_diagnosis_full_hiermean_results_main_full_hiermean_seed42/main_diagnosis_full_hiermean",
    "train_ratio": 0.70,
    "val_ratio": 0.15,
    "seed": 42,
    ...
    # HCM-priority Cox training and model selection.
    "disease_mode": "HCM_only",     # "both_independent" / "joint" / "DCM_only" / "HCM_only"
    ...
    "both_independent_modes": "HCM_only",
    "both_output_root": "auto",
    "both_continue_on_error": False,

    # ---------------- IO ----------------
    "output_dir": "auto",
    "experiment_name": "hcm_nested_oof_fixed_seed42",
    "run_id": "",
    "resume_head": "",
    "eval_only": False,
    "print_freq": 20,
    ...
    # is initialized from it and adapted with HCM survival supervision only.
    ...
    "fullrisk_variants": "A_direct,F_multilevel_residual_b8,S_segmentation_linear,K_oof_late_fusion,L_fold_teacher_ensemble",
    ...
    "cardiac_segmentation_model": "/data/chengyuxi/projects/MRI_New/pretrained_models/ventricular_short_axis_3label/models/model.pt",
    ...
    # ---------------- Final HCM model settings (single source of truth) ----------------
    ...
    "expected_train_n": 705,
    "expected_train_events": 152,
    "expected_val_n": 151,
    "expected_val_events": 39,
    "expected_test_n": 151,
    "expected_test_events": 26,
...
RAW_MULTILEVEL_FEATURE_VERSION = "raw_multilevel_zero_view_embedding_v2"


def multilevel_feature_hash(args, disease: str) -> str:
    feature_version = (
        MULTILEVEL_FEATURE_VERSION
        if str(getattr(args, "classification_ckpt", "") or "").strip()
        else RAW_MULTILEVEL_FEATURE_VERSION
    )
    obj = {
...
def extract_or_load_multilevel(args, split_to_samples, selected_views, required_views, slice_plan, run_dir, device):
    ...
    model.load_classification_checkpoint(args.classification_ckpt)
    if not str(getattr(args, "classification_ckpt", "") or "").strip():
        # A raw/Kinetics encoder has never learned anatomical view identity.
        # Freezing a random view embedding would inject arbitrary view-specific
        # offsets into the matched control, so its deterministic null value is
        # exactly zero.
        with torch.no_grad():
            model.view_embeddings.zero_()
        print0(
            "Raw multilevel control: view_embeddings set deterministically to zero "
            f"(cache_version={RAW_MULTILEVEL_FEATURE_VERSION})"
        )
    model.eval()
