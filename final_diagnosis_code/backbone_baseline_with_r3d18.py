...
RUN_CONFIG = {
    ...
    "visible_gpus": "5,6,7",                  # 单卡 "0"；8卡 "0,1,2,3,4,5,6,7"
    ...
    "experiment_mode": "compare",            # backbone baseline：按 compare_experiments 顺序依次运行
    "compare_continue_on_error": True,        # True: 某个 baseline 失败时继续跑后续实验，并在汇总表中标记缺失
    # "compare_experiments": [
    ...
    "compare_experiments": [
        # Stable pretrained backbone baselines.
        # EfficientNet-B0 is excluded by default because the available local checkpoint
        # uses _conv_stem/_blocks key names and does not match torchvision EfficientNet-B0.
        # Re-enable it only after a compatible model/loader is verified.
        # Fair pretrained backbone baselines:
        # same full-view input, same complete-case cohort, same split, same slice_plan,
        # same num_frames/image_size, same hier_mean fusion; only backbone changes.
        {
            "experiment_name": "exp_videoswin_kinetics_hiermean_full",
        ...
        {
            "experiment_name": "exp_r3d18_kinetics_hiermean_full",
            "model_type": "r3d_18",
            "agg": "hier_mean",
            "pretrained": True,
            "model_weights_path": "/data/projects/MRI/checkpoints/r3d_18-b3b3357e.pth",
            # R3D-18 is substantially heavier at 13x224x224. These are safe starting
            # values, not test-set-tuned optima. Select the final configuration only
            # by validation macro-F1 under the same tuning budget as other backbones.
            "lr": 5e-5,
            "backbone_lr_mult": 0.1,
            "dropout": 0.35,
            "label_smoothing": 0.0,
            ...
            "freeze_3d_bn": True,
            "gradient_checkpoint_backbone": True,
            "backbone_chunk_size": 1,
        },
    ],
    ...
    "model_type": "videoswin",
    "model_name": "",
    "model_weights_path": "",              # 2D/3D CNN 对比模型的本地权重；留空则随机初始化/不加载
    ...
    "pretrained": True,
    "weights_path": "/data/chengyuxi/projects/MRI/checkpoints/swin3d_t-7615ae03.pth",
    "agg": "hier_mean",                    # 正式主实验固定为阶段二确认的最优/主推融合策略
    "dropout": 0.50,
    "freeze_backbone": False,
    "freeze_2d_bn": True,                 # keep BatchNorm stats fixed for 2D CNN baselines
    "freeze_3d_bn": True,                 # keep pretrained BatchNorm3d statistics fixed for R3D-18
    "gradient_checkpoint_backbone": False, # R3D-18 child experiment overrides this to True for memory saving
    ...
    "lr": 5e-5,
    "backbone_lr_mult": 0.1,              # 2D CNN backbone lr = 5e-6; head/aggregation lr = 5e-5
    "min_lr": 1e-6,
    "weight_decay": 1e-4,
    ...
    "early_stop_patience": 12,              # 早停耐心轮数；<=0 表示关闭早停
    ...
    # 例如：./20260717_backbone_baseline_pretrained_hiermean_full_results_20260717_213012/exp_resnet18_imagenet_hiermean_full
    # 注意：run_id 留空才会自动使用当前时间戳；如果 run_id 非空，则输出目录会使用固定 run_id。
    "output_dir": "./final_diagnosis_results/backbone_comparison_runs",
    "experiment_name": "backbone_baselines_pretrained_hiermean_full_fixed_load",  # compare 模式下子实验会使用 compare_experiments 中的名称
    "run_id": "",    # 留空：自动生成 YYYYMMDD_HHMMSS；不要写固定 run_id，否则不会按时间戳新建目录
    "resume": "",
    "eval_only": False,
    "print_freq": 20,
}
...
class MultiView2DCNNClassifier(MultiViewAggregationMixin, nn.Module):
    ...
    def __init__(self, selected_views: Sequence[str], slice_plan: Dict[str, int], num_classes: int = 3,
                 ...
                 freeze_backbone: bool = False, backbone_chunk_size: int = 0, frame_samples: int = 0, freeze_2d_bn: bool = False):
        super().__init__()
        self.selected_views = list(selected_views)
        self.slice_plan = dict(slice_plan)
        self.num_views = len(self.selected_views)
        ...
        self.freeze_2d_bn = bool(freeze_2d_bn)
    ...
    @staticmethod
    def _replace_first_conv_to_1ch(module: nn.Module) -> bool:
        """Replace the first 3-channel Conv2d by a 1-channel Conv2d, preserving geometry.

        This keeps torchvision ImageNet checkpoints usable: load_backbone_weights() will
        average the RGB first-layer weight to one input channel.
        """
        for child_name, child in module.named_children():
            if isinstance(child, nn.Conv2d) and child.in_channels == 3:
                ...
                return True
            if MultiView2DCNNClassifier._replace_first_conv_to_1ch(child):
                return True
        return False

    def _freeze_batchnorm_layers(self):
        """Freeze BN affine parameters and running statistics for 2D CNN baselines.

        With patient-level training, batch_size is typically 1 per GPU. Although each
        patient contributes many frames, the BN batch is still patient-biased and can
        make validation unstable. This keeps ImageNet BN statistics fixed while allowing
        convolutional weights, view embeddings, aggregation, and classifier layers to train.
        """
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        if getattr(self, "freeze_2d_bn", False):
            for m in self.backbone.modules():
                if isinstance(m, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.SyncBatchNorm)):
                    m.eval()
        return self
    ...
    def _create_2d_backbone(self, name: str):
        ...
        if name == "efficientnet_b0":
            # Use torchvision EfficientNet-B0 so that efficientnet-b0-355c32eb.pth can be loaded.
            model = tv_models.efficientnet_b0(weights=None)
            feature_dim = model.classifier[1].in_features
            ok = self._replace_first_conv_to_1ch(model)
            if not ok:
                raise RuntimeError("Failed to replace EfficientNet-B0 first conv to 1-channel input.")
            model.classifier = nn.Identity()
            return model, feature_dim
        if name == "convnext_tiny":
            # Use torchvision ConvNeXt-Tiny when a matching local torchvision checkpoint is available.
            model = tv_models.convnext_tiny(weights=None)
            feature_dim = model.classifier[2].in_features
            ok = self._replace_first_conv_to_1ch(model)
            if not ok:
                raise RuntimeError("Failed to replace ConvNeXt-Tiny first conv to 1-channel input.")
            model.classifier = nn.Identity()
            return model, feature_dim
        raise ValueError(f"Unsupported 2D backbone: {name}")

    @staticmethod
    def _fix_densenet_key(key: str) -> str:
        """Convert old torchvision DenseNet keys: norm.1.weight -> norm1.weight."""
        pattern = re.compile(
    ...
    def load_backbone_weights(self, weights_path: str):
        p = Path(weights_path)
        if not p.exists():
            print0(f"⚠️ 找不到 2D backbone 权重文件: {p}，将从随机初始化训练")
            return

        print0(f"🚀 加载本地 2D backbone 权重: {p}")
        # safe_torch_load is defined earlier in this file; it avoids the FutureWarning when available.
        ...
        for k, v in state.items():
            ...
            # torchvision DenseNet older checkpoints use denselayer1.norm.1.weight style,
            # while the current torchvision model expects denselayer1.norm1.weight.
            if self.backbone_name == "densenet121":
                kk = self._fix_densenet_key(kk)

            # Remove task-specific classification heads from ImageNet checkpoints.
            # Our baseline replaces these heads by nn.Identity() and uses a new CMR classifier.
            if (
                kk.startswith("fc.")
                or kk.startswith("classifier.")
                or kk.startswith("head.")
                or ".head." in kk
            ):
                skipped_head.append(kk)
                continue

            # Convert RGB ImageNet first-layer filters into single-channel CMR filters.
            # This is safe for conv1, DenseNet conv0, EfficientNet features.0.0, ConvNeXt stem, etc.
            if hasattr(v, "ndim") and v.ndim == 4 and v.shape[1] == 3:
                v = v.mean(dim=1, keepdim=True)

            # Only keep tensors that actually belong to the current backbone and match shape after conversion.
            if kk not in model_state:
                skipped_not_in_model.append(kk)
                continue
            if tuple(v.shape) != tuple(model_state[kk].shape):
                skipped_shape.append((kk, tuple(v.shape), tuple(model_state[kk].shape)))
                continue
            clean[kk] = v

        msg = self.backbone.load_state_dict(clean, strict=False)
        print0(
            f"🎉 2D 权重加载完成: loaded={len(clean)}, "
            f"missing={len(msg.missing_keys)}, unexpected={len(msg.unexpected_keys)}, "
            f"skipped_head={len(skipped_head)}, skipped_shape={len(skipped_shape)}, skipped_not_in_model={len(skipped_not_in_model)}"
        )
        ...
        if len(skipped_shape) > 0:
            print0(f"  skipped_shape examples: {skipped_shape[:5]}")
        if len(skipped_not_in_model) > 0:
            print0(f"  skipped_not_in_model examples: {skipped_not_in_model[:8]}")
...
class MultiView3DCNNClassifier(MultiViewAggregationMixin, nn.Module):
    """3D CNN baseline using the same patient -> view -> slice aggregation pipeline.

    Each selected slice cine clip [1,T,H,W] is repeated to three channels and encoded
    by a Kinetics-pretrained torchvision video backbone. R3D-18 is the formal 3D CNN
    baseline; the classifier and hier_mean fusion are identical to the main framework.
    """
    ...
    def __init__(
        ...
        dropout: float = 0.35,
        freeze_backbone: bool = False,
        backbone_chunk_size: int = 1,
        freeze_3d_bn: bool = True,
        gradient_checkpoint_backbone: bool = True,
    ):
        super().__init__()
        self.selected_views = list(selected_views)
        self.slice_plan = dict(slice_plan)
        ...
        self.backbone_chunk_size = max(1, int(backbone_chunk_size or 1))
        self.freeze_3d_bn = bool(freeze_3d_bn)
        self.gradient_checkpoint_backbone = bool(gradient_checkpoint_backbone)

        if self.backbone_name not in {"r3d_18", "r3d18"}:
            raise ValueError(f"Unsupported 3D backbone: {self.backbone_name}")
        ...
        if pretrained and weights_path:
            ...
            print0("⚠️ R3D-18 pretrained=True but model_weights_path is empty; using random initialization.")

        if freeze_backbone:
            for p in self.backbone.parameters():
                p.requires_grad = False

        if self.freeze_3d_bn:
            self._freeze_batchnorm_layers()
        ...
        print0(
            f"✅ MultiView R3D-18 initialized: views={self.selected_views}, nclips={self.nclips}, "
            f"dim={self.feature_dim}, agg={self.agg}, freeze_3d_bn={self.freeze_3d_bn}, "
            f"gradient_checkpoint={self.gradient_checkpoint_backbone}, chunk={self.backbone_chunk_size}"
        )

    def _build_view_clip_ranges(self) -> Dict[int, Tuple[int, int]]:
        ranges, start = {}, 0
        for local_vid, view in enumerate(self.selected_views):
            k = int(self.slice_plan.get(view, 0))
            ranges[local_vid] = (start, start + k)
            start += k
        return ranges
    ...
    def train(self, mode: bool = True):
        super().train(mode)
        if getattr(self, "freeze_3d_bn", False):
            for m in self.backbone.modules():
                if isinstance(m, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.SyncBatchNorm)):
                    m.eval()
        return self
    ...
    def load_backbone_weights(self, weights_path: str):
        p = Path(weights_path)
        if not p.exists():
            print0(f"⚠️ R3D-18 weights not found: {p}; using random initialization.")
            return

        print0(f"🚀 Loading local R3D-18 Kinetics weights: {p}")
        ckpt = safe_torch_load(str(p), map_location="cpu", weights_only=True)
        state = ckpt.get("model", ckpt.get("state_dict", ckpt)) if isinstance(ckpt, dict) else ckpt
        model_state = self.backbone.state_dict()
        clean = {}
        ...
        for key, value in state.items():
            ...
            for prefix in ("module.", "backbone.", "model."):
                if kk.startswith(prefix):
                    kk = kk[len(prefix):]
            if kk.startswith("fc.") or kk.startswith("head.") or kk.startswith("classifier."):
                continue
            if kk not in model_state or tuple(value.shape) != tuple(model_state[kk].shape):
                skipped.append(kk)
                continue
            clean[kk] = value
        ...
        msg = self.backbone.load_state_dict(clean, strict=False)
        print0(
            f"🎉 R3D-18 weights loaded: loaded={len(clean)}, missing={len(msg.missing_keys)}, "
            f"unexpected={len(msg.unexpected_keys)}, skipped={len(skipped)}"
        )
        if msg.missing_keys:
            print0(f"  missing examples: {msg.missing_keys[:8]}")
    ...
    def _forward_backbone_chunk(self, x: torch.Tensor) -> torch.Tensor:
        use_ckpt = (
            self.training
            and self.gradient_checkpoint_backbone
            and any(p.requires_grad for p in self.backbone.parameters())
        )
        if use_ckpt:
            return gradient_checkpoint(self.backbone, x, use_reentrant=False)
        return self.backbone(x)

    def _run_backbone(self, x: torch.Tensor) -> torch.Tensor:
        chunk = max(1, int(self.backbone_chunk_size))
        if x.shape[0] <= chunk:
            return self._forward_backbone_chunk(x)
        outputs = []
        for start in range(0, x.shape[0], chunk):
            outputs.append(self._forward_backbone_chunk(x[start:start + chunk]))
    ...
    def _extract_clip_features(self, batch: Dict) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        B, N, C, T, H, W = videos.shape
        if N != self.nclips:
            raise RuntimeError(f"input Nclips={N}, model nclips={self.nclips}")
        ...
        if flat_mask.any():
            x_valid = flat_videos[flat_mask]
            # Keep the same normalized grayscale input as the other baselines, but
            # repeat it to the three channels required by torchvision video models.
            x_valid = x_valid.expand(-1, 3, -1, -1, -1).contiguous()
            feat_valid = self._run_backbone(x_valid).float()
            feat_valid = feat_valid + self.view_embeddings[flat_view_ids[flat_mask]]
            feats_flat[flat_mask] = feat_valid
    ...
    def forward_features(self, batch: Dict) -> torch.Tensor:
        feats, clip_mask = self._extract_clip_features(batch)
        patient_feat, _ = self._aggregate_features(feats, clip_mask)
        return patient_feat
...
def build_model(args, selected_views: Sequence[str], slice_plan: Dict[str, int]) -> nn.Module:
    ...
    if model_type in {"r3d_18", "r3d18", "cnn3d"}:
        return MultiView3DCNNClassifier(
            selected_views=selected_views,
            slice_plan=slice_plan,
            num_classes=3,
            backbone_name="r3d_18",
            weights_path=getattr(args, "model_weights_path", "") or None,
            pretrained=args.pretrained,
            agg=args.agg,
            dropout=args.dropout,
            freeze_backbone=args.freeze_backbone,
            backbone_chunk_size=args.backbone_chunk_size,
            freeze_3d_bn=getattr(args, "freeze_3d_bn", True),
            gradient_checkpoint_backbone=getattr(args, "gradient_checkpoint_backbone", True),
        )
    ...
    if backbone_name in {"resnet18", "resnet50", "densenet121", "efficientnet_b0", "convnext_tiny"}:
        return MultiView2DCNNClassifier(selected_views=selected_views, slice_plan=slice_plan, num_classes=3,
            ...
            frame_samples=args.frame_samples, freeze_2d_bn=getattr(args, "freeze_2d_bn", False))
...
def _get_gradcam_target_layer(target_model: nn.Module):
    ...
    if isinstance(target_model, MultiView3DCNNClassifier):
        return target_model.backbone.layer4[-1], "cnn3d"
...
def _gradcam_for_selected_clip(target_model: nn.Module, batch: Dict, clip_idx: int, target_class: Optional[int] = None) -> Optional[np.ndarray]:
    ...
    try:
        ...
        if family == "cnn3d":
            if act.dim() != 5 or grad.dim() != 5:
                return None
            idx = min(valid_order, act.shape[0] - 1)
...
def _comparison_root_dir(run_id: str) -> Path:
    ...
    return Path(output_dir) / f"backbone_comparison_{run_id}"
...
def maybe_run_compare_parent():
    ...
    for idx, exp in enumerate(experiments, 1):
        ...
        try:
            ...
            print(f"[ERROR] 实验失败: {exp_name} | returncode={e.returncode}")
            if not bool(RUN_CONFIG.get("compare_continue_on_error", False)):
                raise
            print("[WARN] compare_continue_on_error=True，继续运行后续 baseline。")
...
def parse_args():
    ...
    # Model
    parser.add_argument("--model_type", type=str, default=cfg.get("model_type", "videoswin"), help="videoswin / r3d_18 / resnet18 / resnet50 / densenet121 / efficientnet_b0 / convnext_tiny")
    ...
    parser.add_argument("--model_weights_path", type=str, default=cfg.get("model_weights_path", ""), help="Local 2D/3D backbone weight path for comparison models")
    ...
    add_bool_arg(parser, "freeze_2d_bn", cfg.get("freeze_2d_bn", True), "Freeze BatchNorm stats/affine parameters for 2D CNN baselines")
    add_bool_arg(parser, "freeze_3d_bn", cfg.get("freeze_3d_bn", True), "Freeze BatchNorm3d stats/affine parameters for R3D-18")
    add_bool_arg(parser, "gradient_checkpoint_backbone", cfg.get("gradient_checkpoint_backbone", False), "Use activation checkpointing for the R3D-18 backbone")
    ...
    parser.add_argument("--run_id", type=str, default=cfg.get("run_id", ""),
                        help="Optional fixed run id. Empty means current timestamp and enables output_dir='auto' timestamp naming.")
...
def main():
    ...
    model_profile.update({
        ...
        "freeze_2d_bn": bool(getattr(args, "freeze_2d_bn", False)),
        "freeze_3d_bn": bool(getattr(args, "freeze_3d_bn", False)),
        "gradient_checkpoint_backbone": bool(getattr(args, "gradient_checkpoint_backbone", False)),
