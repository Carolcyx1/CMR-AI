...
RUN_CONFIG = {
    ...
    "experiment_mode": "compare",           # 多 seed 稳定性：按 compare_experiments 顺序跑 seed2024/seed3407
    # "compare_experiments": [
    ...
    "compare_experiments": [
        # 多 seed 稳定性：复用 seed42 的 train/val/test split，只改变训练随机性。
        {
            "experiment_name": "main_diagnosis_full_hiermean_seed2024",
            "seed": 2024,
        ...
        {
            "experiment_name": "main_diagnosis_full_hiermean_seed3407",
            "seed": 3407,
            "model_type": "videoswin",
            "agg": "hier_mean",
            "selected_views": "Cine2CH,Cine3CH,Cine4CH,CineSAX",
    ...
    # ---------------- Fixed split for multi-seed stability ----------------
    # 多 seed 稳定性必须复用同一套 split，而不是换 seed 后重新切分数据。
    # 默认填 seed42 正式主实验目录；脚本会读取其中的 split_train.csv / split_val.csv / split_test.csv。
    "fixed_split_dir":""
    "fixed_split_train_csv": "",
    "fixed_split_val_csv": "",
    "fixed_split_test_csv": "",
    ...
    "output_dir": "./final_diagnosis_results/multiseed_stability_runs",
    "experiment_name": "main_diagnosis_full_hiermean_multiseed_fixedsplit",  # compare 子实验会覆盖为 main_diagnosis_full_hiermean_seedXXXX
    "run_id": "",    # 多 seed 稳定性共同 run id
    "resume": "",
    "eval_only": False,
    "print_freq": 20,
}
...
def _split_csv_paths_from_args(args) -> Tuple[Optional[Path], Optional[Path], Optional[Path]]:
    """Resolve optional fixed split CSV paths from args/RUN_CONFIG."""
    train_csv = str(getattr(args, "fixed_split_train_csv", "") or "").strip()
    val_csv = str(getattr(args, "fixed_split_val_csv", "") or "").strip()
    test_csv = str(getattr(args, "fixed_split_test_csv", "") or "").strip()
    fixed_dir = str(getattr(args, "fixed_split_dir", "") or "").strip()
    if fixed_dir and not (train_csv or val_csv or test_csv):
        d = Path(fixed_dir)
        train_csv = str(d / "split_train.csv")
        val_csv = str(d / "split_val.csv")
        test_csv = str(d / "split_test.csv")
    if not (train_csv or val_csv or test_csv):
        return None, None, None
    if not (train_csv and val_csv and test_csv):
        raise ValueError("fixed split requires train/val/test CSV paths together, or fixed_split_dir.")
    return Path(train_csv), Path(val_csv), Path(test_csv)


def load_split_samples_from_csv(csv_path: Path, all_samples: List[Dict]) -> List[Dict]:
    """
    Load split CSV saved by save_split_csv and map rows back to scanned samples.
    This preserves the original organized_data and file paths from the current data scan.
    """
    if not csv_path.exists():
        raise FileNotFoundError(f"fixed split CSV not found: {csv_path}")
    df = pd.read_csv(csv_path)
    required_cols = {"patient_id", "class_name"}
    missing = required_cols - set(df.columns)
    if missing:
        raise ValueError(f"fixed split CSV {csv_path} missing columns: {sorted(missing)}")

    by_pair: Dict[Tuple[str, str], Dict] = {}
    for s in all_samples:
        key = (str(s.get("class_name", "")), str(s.get("patient_id", "")))
        by_pair[key] = s

    out: List[Dict] = []
    missing_rows: List[Tuple[str, str]] = []
    for _, row in df.iterrows():
        key = (str(row["class_name"]), str(row["patient_id"]))
        s = by_pair.get(key)
        if s is None:
    ...
    if missing_rows:
        preview = ", ".join([f"{c}/{p}" for c, p in missing_rows[:10]])
        raise RuntimeError(
            f"{len(missing_rows)} samples in fixed split CSV are not found in current scanned cohort. "
            f"Examples: {preview}. Check data_path/cohort_required_views/selected_views."
        )
    return out
...
def _comparison_root_dir(run_id: str) -> Path:
    ...
    return Path(output_dir) / f"multiseed_stability_{run_id}"
...
def parse_args():
    ...
    parser.add_argument("--fixed_split_dir", type=str, default=cfg.get("fixed_split_dir", ""),
                        help="Directory containing split_train.csv/split_val.csv/split_test.csv. If set, seed changes only training randomness, not the data split.")
    parser.add_argument("--fixed_split_train_csv", type=str, default=cfg.get("fixed_split_train_csv", ""))
    parser.add_argument("--fixed_split_val_csv", type=str, default=cfg.get("fixed_split_val_csv", ""))
    parser.add_argument("--fixed_split_test_csv", type=str, default=cfg.get("fixed_split_test_csv", ""))
...
def main():
    ...
    fixed_train_csv, fixed_val_csv, fixed_test_csv = _split_csv_paths_from_args(args)
    if fixed_train_csv is not None:
        train_samples = load_split_samples_from_csv(fixed_train_csv, base_dataset.samples)
        val_samples = load_split_samples_from_csv(fixed_val_csv, base_dataset.samples)
        test_samples = load_split_samples_from_csv(fixed_test_csv, base_dataset.samples)
        print0("✅ 使用固定 split CSV，不随 seed 重新划分数据：")
        print0(f"  train_csv = {fixed_train_csv}")
        print0(f"  val_csv   = {fixed_val_csv}")
        print0(f"  test_csv  = {fixed_test_csv}")
    else:
